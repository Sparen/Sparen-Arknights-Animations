...
# Usage:
# colorcorrectandspritify.py <input filename (GIF)> <output filename (PNG)> <channel to use, with 0 being red, 1 being green, and 2 being blue> <thresholding parameter (int, 0-255*3 scale)>
...
# 2. Adjust the channel by the difference (IE set it to the average) and alpha by the difference
...
def process_boundary_pixel(outimg, x, y, destoffsetx, destoffsety):
	...
	newpixel = currpixel
	newpixel[channel] = int(avgvalue)
	outimg.putpixel((destoffsetx + x, destoffsety + y), (newpixel[0], newpixel[1], newpixel[2], 255 - int(difference)))
	...
	for i in range(inimg.n_frames):
		...
		for y in range(width):
			for x in range(height):
				...
				if matrix[x][y] == 0 and neighbor_matches_mask(x, y, matrix): 
					matrix[x][y] = 1
		...
		for y in range(width):
			for x in range(height):
				if matrix[x][y] == 1: # Adjust the specific channel
					process_boundary_pixel(outimg, x, y, destoffsetx, destoffsety)
